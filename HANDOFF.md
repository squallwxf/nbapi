...
- 已完全撤回自动请求压缩以及 NBAPI_UPSTREAM_REQUEST_GZIP 开关，恢复原始请求正文发送。保留日志标签和时序诊断，不修改余额、API Key、计费公式与数据库。下文默认启用 gzip 的说明已作废，禁止未经实际渠道验证再次默认开启。
- 操练场显式使用 stream:false，非流式请求的首字显示 - 是正常行为，不表示调用失败。不得用非流式小请求验证长上下文流式链路。
- 16 项测试通过，包括真实 HTTP 大 JSON 原文传输检查、流式读取和用量解析。SSH 连接仍被服务器关闭，需服务器拉取此恢复提交并重启后验证真实站外调用；当前不能声称已恢复生产。

## 2026-09-12 首字时间起点修复

- 已确认上一版首字时间偏大的根因：`first_token_ms` 使用端到端 `started_at`，把 NBAPI 接收/上传长上下文的时间也算进了首字。
- 现在首字/FRT 从 NBAPI 开始向选定上游发送请求时计时，与 New API 在渠道转发阶段记录 FRT 的口径一致；总用时仍从收到客户端请求开始计时，计费和调用逻辑不变。
- 旧日志的首字数值不会回写，新请求部署后才使用修复口径。若上游响应头本身等待较久，首字仍可能较大，但不再包含客户端到 NBAPI 的上传阶段。
- 已通过 16 项回归测试、Python 语法检查和 `git diff --check`。
